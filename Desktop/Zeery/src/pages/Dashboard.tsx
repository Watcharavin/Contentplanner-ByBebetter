...
export default function Dashboard() {
  ...
  const expense = Math.abs(periodTxs.filter(t => t.amount < 0).reduce((s, t) => s + t.amount, 0))
  ...
  const balancePct = income > 0 ? Math.max(0, Math.min(1, balance / income)) : 0
  ...
  const budgetItems = budgets
    .map((b: Budget) => ({ ...b, spent: spentByCat[b.catId] || 0, cat: getCategoryById(b.catId) }))
              ...
              return (
                <div key={g.id}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text)', fontWeight: 500 }}>{g.name}</span>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text2)' }}>{calcETA(g)}</span>
                  </div>
                  <DotChart type="fill" pct={pct} accent="var(--purple)" height={32} />
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text2)', fontFamily: 'DM Mono, monospace' }}>
                      ฿{fmt(g.saved)}
                    </span>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text2)', fontFamily: 'DM Mono, monospace' }}>
                      ฿{fmt(g.target)}
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: '0.7rem', color: 'var(--text2)' }}>{tx.date}</p>
                  </div>
                  <span
                    style={{
                      fontFamily: 'DM Mono, monospace',
